...
export const STORED_ORDER_SCHEMA = z.object({
  ...
  // hard delete would leave gaps / risk number reuse). Optional so orders
  // written before this field stay valid. Distinct from "cancelled", which is a
  // shipment status that keeps the order visible.
