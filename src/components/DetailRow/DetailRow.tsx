...
//
// `labelBasisClass` sets the label column width FROM `sm:` up (must carry the
// `sm:` prefix so it's a literal Tailwind can emit): the order page uses wide
// labels ("Delivery method") so it keeps the 200px default; the customer page
// passes a narrower basis. The value carries `break-words` so a long unbroken
// string (e.g. an address) wraps instead of overflowing — a no-op for ordinary
// content.
//
// Layout is RESPONSIVE: on a phone the row STACKS (label on its own line, value
// below) so a fixed 200px label can't squeeze the value down to
...
const DetailRow = ({
  ...
  <div className="flex flex-col gap-1 border-b border-border py-2 sm:flex-row sm:items-center sm:gap-3">
    ...
    <div className="min-w-0 break-words text-heading sm:flex-1">{value}</div>
  </div>
)
